class ControllerSpent {
    ...
    getAll = async (req, res) => {
        try {
            res.status(200).json(await this.container.getUserSpent(req.user.id))
        } catch (error) {
    ...
    getCant = async (req, res) => {
        try {
            let cant = 0
            const incomes = await this.container.getUserSpentCant(req.user.id)
            incomes.map(item => (
...
export default ControllerSpent